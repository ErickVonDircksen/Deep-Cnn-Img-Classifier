...
#-------- tensorflow v2.11.0-----cuda v11.0.194-------cDNN v8.0-------#
...
#loading com keras
data = tf.keras.utils.image_dataset_from_directory('data',batch_size=64,shuffle=True) # Keras cria um dataset com labels, resized images,batch size etc... 
...
mc = tf.keras.callbacks.ModelCheckpoint(os.path.join('models','best_model-TEST.h5'), monitor='val_accuracy', mode='max', verbose=1, save_best_only=True)

hist = model.fit(train, epochs=20, validation_data=val,verbose="auto",callbacks=[mc]) # treinamento 
